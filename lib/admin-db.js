  ...
  try {
    ...
    if (tableName === 'registration_details') {
      const allowedColumns = [
        ...
        'document_url', 'partner_document_url', 'payment_screenshot_url',
      ];
      const filtered = {};
    ...
    const result = await client.query(
      `UPDATE registrations
         SET registration_status = 'confirmed',
             payment_status = CASE WHEN payment_required THEN 'paid' ELSE 'not_required' END,
             updated_at = NOW()
       WHERE id = $1
       RETURNING id`,
      [registrationId]
    ...
    if (result.rowCount > 0) {
      try {
        const detailRes = await client.query(
          `SELECT r.full_name, r.email, r.bib_number, r.mobile_no, rc.label AS category_label
        ...
        if (detailRes.rowCount > 0) {
          const { full_name, email, bib_number, mobile_no, category_label } = detailRes.rows[0];
...
export async function rejectParticipation(registrationId, reason) {
  const client = await pool.connect();
  try {
    const result = await client.query(
      `UPDATE registrations
    ...
    if (result.rowCount > 0) {
      try {
        const detailRes = await client.query(
          `SELECT r.full_name, r.email, r.mobile_no, rc.label AS category_label
           FROM registrations r
           JOIN race_categories rc ON r.category_code = rc.code
           WHERE r.id = $1`,
          [registrationId]
        ...
        if (detailRes.rowCount > 0) {
          const { full_name, email, mobile_no, category_label } = detailRes.rows[0];
